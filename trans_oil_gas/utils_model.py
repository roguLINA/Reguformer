...
class SiameseArchitecture(nn.Module):
    ...
    def __init__(
        ...
        **specific_encoder_params: Dict[str, Any],
        ...
        :param encoder_type: type of transformer-based encoder ('transformer', 'reguformer', and 'performer' are available). To use transformer with the same way as reguformer and without positional encoding, use encoder_type='reguformer', specify specific_encoder_params['attn']='full' and specific_encoder_params['activation']='relu'. In case of encoder_type='transformer', the variant from PyTorch would be implemented
        ...
        if "reguformer" in encoder_type:
            self.positional_encoding = None
            self.encoder = reguformer.ReguformerEncoder(
        ...
        if "dropdim" in encoder_type:
            ...
            in_emb = specific_encoder_params["dim"] 
    ...
    def get_attention_maps(
        ...
        """
        if add_positional_encoding and "transformer" in self.encoder_type:
...
class TripletArchitecture(nn.Module):
    ...
    def __init__(
        ...
        elif "reguformer" in encoder_type:
            self.positional_encoding = None
            self.encoder = reguformer.ReguformerEncoder(
                enc_in=specific_encoder_params["enc_in"],
                factor=specific_encoder_params["factor"],
